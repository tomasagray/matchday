...
class GalatamanPattern
{
    ...
    // -------------------------------------------------------------------------
    static final String START_OF_SOURCE = 
            Pattern.compile( "__*" ).pattern();
    ...
    static final String LANGUAGE_DELIMITER = 
            Pattern.compile( "[\\d.* ]|/" ).pattern();
    
    static final String AV_DATA_DELIMITER =
    ...
    static final Predicate<Element> isSourceData = 
        elem ->
                ( "b".equals(elem.tagName()) )
                        &&
                ( elem.text().contains("Channel") );
    ...
    static final Predicate<Element> isVideoLink = 
        elem ->
                ...
                (
                    elem.attr("href")
                        .startsWith("https://www.inclouddrive.com/file/")
                );
}
