...
public class VideoStreamer {
    ...
    private static final OpenOption[] LOG_WRITE_OPTIONS = {StandardOpenOption.CREATE, StandardOpenOption.WRITE};

    private final VideoFileService videoFileService;
    private final VideoStreamLocatorService locatorService;
    private final FFmpegPlugin ffmpegPlugin;

    public VideoStreamer(VideoFileService videoFileService, VideoStreamLocatorService locatorService, FFmpegPlugin ffmpegPlugin) {
    ...
    public CompletableFuture<Long> beginStreaming(@NotNull VideoStreamLocator streamLocator, Runnable onComplete) {
        try {
    ...
    private void streamWithLogging(@NotNull VideoStreamLocator streamLocator, @NotNull FFmpegStreamTask streamTask)
            throws IOException {
        ...
        final FFmpegLogAdapter logReader = new FFmpegLogAdapter(streamLocator, this::updateLocatorTaskState);

        AsynchronousFileChannel fw = AsynchronousFileChannel.open(logger.getLogFile(), LOG_WRITE_OPTIONS);
        final Flux<String> logEmitter = logger.beginLogging(fw);
        logEmitter.doOnNext(logReader)
                .doOnComplete(
                        () -> {
                            JobStatus previousStatus =
                                    locatorService.getStreamLocator(locatorId)
    ...
    private void streamWithoutLog(@NotNull final FFmpegStreamTask streamTask) throws IOException, InterruptedException {
    ...
    public void updateLocatorTaskState(@NotNull VideoStreamLocator streamLocator, @NotNull TaskState state) {
    ...
    private void setLocatorErrorState(@NotNull VideoStreamLocator streamLocator, @NotNull Throwable error) {
