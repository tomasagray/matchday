...
public class FileServerPluginService {
    ...
    FileServerPluginService(
            ...
            SecureDataService secureDataService, PluginService pluginService) {
    ...
    public void enablePlugin(@NotNull final UUID pluginId) {
        getPluginById(pluginId)
                .ifPresentOrElse(plugin -> pluginService.enablePlugin(plugin, ENABLED_FILESERVERS),
                        () -> {
                            throw new PluginNotFoundException("Trying to enable non-existent FileServerPlugin: " + pluginId);
                        });
    }
    ...
    public void disablePlugin(@NotNull final UUID pluginId) {
        getPluginById(pluginId)
                .ifPresentOrElse(plugin -> pluginService.disablePlugin(plugin, ENABLED_FILESERVERS),
                        () -> {
                            throw new PluginNotFoundException("Cannot disable non-existent FileServerPlugin: " + pluginId);
                        }
                );
    }
