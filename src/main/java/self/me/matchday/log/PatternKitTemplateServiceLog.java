...
public class PatternKitTemplateServiceLog {
    ...
    @Around("execution(* self.me.matchday.api.service.PatternKitTemplateService.fetchAll(..))")
    ...
    @Before("execution(* self.me.matchday.api.service.PatternKitTemplateService.fetchById(..))")
    public void logFetchTemplateById(@NotNull JoinPoint jp) {
        logger.info("Retrieving PatternKitTemplate for ID: {}", jp.getArgs()[0]);
    }

    @Around("execution(* self.me.matchday.api.service.PatternKitTemplateService.fetchByClassName(..))")
    ...
    @Before("execution(* self.me.matchday.api.service.PatternKitTemplateService.save(..))")
    public void logSavePatternKitTemplate(@NotNull JoinPoint jp){
        logger.info("Saving PatternKitTemplate: {}", jp.getArgs()[0]);
    }

    @Before("execution(* self.me.matchday.api.service.PatternKitTemplateService.update(..))")
    public void logUpdatePatternKitTemplate(@NotNull JoinPoint jp) {
        logger.info("Updating PatternKitTemplate to: {}", jp.getArgs()[0]);
    }

    @Before("execution(* self.me.matchday.api.service.PatternKitTemplateService.delete(..))")
