...
public class ForumPlugin implements DataSourcePlugin {
    ...
    private static String getQuery(@NotNull Map<String, List<String>> params) {
        return params.entrySet().stream()
                .flatMap(param -> param.getValue().stream().map(v -> String.format("%s=%s", param.getKey(), v)))
    ...
    public <T> Snapshot<T> getSnapshot(@NotNull SnapshotRequest request, @NotNull DataSource<T> dataSource)
        ...
        int scrapeSteps = pluginProperties.getScrapeSteps() - 1; // subtract 1 to account for the initial 'do' loop
    ...
    @SuppressWarnings("unchecked cast")
    private <T> @NotNull Stream<T> getEventStream(URL url, DataSource<? extends Event> eventDataSource)
            throws IOException {
    ...
    private Event readEvent(@NotNull URL url, @NotNull DataSource<? extends Event> dataSource) throws IOException {
        // follow link
