...
public class ZKFFileMetadata extends FileSourceMetadata {

  private static final String LOG_TAG = "ZKFFileMetadata";
  ...
  ZKFFileMetadata(@NotNull final Elements elements, @NotNull final ZKFPatterns zkfPatterns) {
  ...
  private void parseEventMetadata(@NotNull final Elements elements) {
    ...
    elements.forEach(
        element -> {
          ...
          switch (text) {
            case CHANNEL:
              final String channel = parseChannel(element);
              setChannel(channel);
              break;

            case LANGUAGE:
              final String languages = parseLanguages(element);
            ...
            case BITRATE:
              final long bitrate = parseBitrate(element);
              setBitrate(bitrate);
              break;

            case SIZE:
              // Set file size from final element
              final Long fileSize = parseFileSize(element);
    ...
    try {
      for (String part : format) {
        ...
        if (part.contains("4096x2160")) {
          setResolution(EventFileSource.Resolution.R_4k);
          // Other resolutions
        } else if (zkfPatterns.getResolutionMatcher(part).find()) {
          setResolution(EventFileSource.Resolution.fromString(part));
        } else {
          ...
          if (frMatcher.find()) {
            // Parse frame rate
            final int frameRate = Integer.parseInt(frMatcher.group(1));
            setFrameRate(frameRate);

          } else if (zkfPatterns.getContainerMatcher(part).find()) {
            // Media container
            final String mediaContainer = part.toUpperCase();
      ...
      Log.d(
          LOG_TAG, String.format("Could not parse format data from supplied String: %s", data), e);
    }
  }
