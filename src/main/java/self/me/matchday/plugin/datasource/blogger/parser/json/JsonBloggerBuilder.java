...
public class JsonBloggerBuilder implements BloggerBuilder {
  ...
  private final String blogData;

  private JsonObject feed;
  private long postCount;

  // Constructor
  public JsonBloggerBuilder(
      @NotNull final String blogData, @NotNull final JsonPostBuilderFactory postBuilderFactory) {
  ...
  private String parseId() {
    ...
    try {
      String formattedId = feed.get("id").getAsJsonObject().get("$t").getAsString();
  ...
  private String parseTitle() {
    ...
    try {
      title = this.feed.get("title").getAsJsonObject().get("$t").getAsString();
    } catch (NullPointerException e) {
  ...
  public Blogger getBlogger() {
    ...
    final JsonObject json = JsonStreamReader.readJsonString(this.blogData);
    ...
    return new Blogger(blogId, title, version, author, link, posts, postCount);
  }
}
