...
public class ScheduledTaskServiceLog {
  ...
  @AfterReturning(
      value =
          "execution(* net.tomasbot.matchday.api.service.ScheduledTaskService.getLatestEventDate(..))",
      returning = "latest")
  public void logGetLatestEventDate(@NotNull Object latest) {
    logger.info("Found latest Event date for refresh: {}", latest);
  }

  @Before("execution(* net.tomasbot.matchday.api.service.ScheduledTaskService.pruneVideoData())")
  public void logPruneVideoData() {
    logger.info("Pruning video data more than 2 weeks old...");
  }

  @Around(
      "execution(* net.tomasbot.matchday.api.service.ScheduledTaskService.isVideoDataStale(..))")
  ...
  @Before("execution(* net.tomasbot.matchday.api.service.ScheduledTaskService.rescheduleTasks(..))")
  public void logRescheduleTasks() {
    logger.info("Rescheduling tasks...");
  }

  @Before("execution(* net.tomasbot.matchday.api.service.ScheduledTaskService.scheduleTasks(..))")
