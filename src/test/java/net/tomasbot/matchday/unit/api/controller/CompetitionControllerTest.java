...
class CompetitionControllerTest {
  ...
  private Stream<Arguments> getAllTestCompetitionsArgs() {
    final String url = "http://localhost:" + port + "/api/v1/competitions/";
  ...
  private ResponseEntity<CompetitionResource> getCompetition(@NotNull UUID competitionId) {
    final String url =
        String.format(
            "http://localhost:%d/api/v1/competitions/competition/%s", port, competitionId);
  ...
  void fetchCompetitionTeams(@NotNull Competition competition) {
    final String url =
        String.format(
            "http://localhost:%d/api/v1/competitions/competition/%s/teams",
            port, competition.getId());
  ...
  void fetchCompetitionEvents(@NotNull Competition competition) {
    final String url =
        String.format(
            "http://localhost:%d/api/v1/competitions/competition/%s/events",
            port, competition.getId());
